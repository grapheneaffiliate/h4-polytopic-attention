"""
Unified ARC-AGI-3 Explorer v4 — speed-optimized, adaptive grid-click.

Key improvements over unified v1 (explorer_unified.py):
1. ~3x faster hashing: avoid frame copy when possible, use memoryview
2. ~5x faster segmentation: scipy.ndimage.label with C-level connected components
3. Smarter grid-click: lower thresholds (30 states / 3K actions), adaptive grid step
4. O(1) reverse action lookup instead of O(n) scan per step
5. Reduced redundant process_frame calls in game loop
6. Higher default budget: 200K actions
7. Multi-step grid-click escalation: step=4 → step=2 if still stuck
"""
...
def solve_game(arc, game_id, max_actions=200000, verbose=True):
    ...
    agent = UnifiedAgentV4(grid_step=4)
