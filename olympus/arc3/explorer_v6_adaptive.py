"""
ARC-AGI-3 Explorer v6 — adaptive budget, action classifier, replay optimization.

All v5 features (UCB1, efficacy-based mode switching) PLUS:
1. Frame diff action classifier: after 30 actions, knows what each action does
2. Variable budget per game: 500K for high-potential, 150K for easy solves
3. Replay path optimization: only replay state-changing actions
4. Per-game retry: if 0 levels at 100K, try other mode for remaining budget
"""
...
class UnifiedAgentV6:
    ...
    def process_frame_grid(self, frame, available_actions):
        ...
        if not self.click_grid:
            H, W = frame.shape
            self._build_click_grid(H, W, self.grid_step)
...
# Per-game budget allocation (#2)
# High-potential games get more budget. Games that solve quickly get less.
GAME_BUDGETS = {
    # Top performers — give them more budget to find more levels
    "lp85": 400000,   # 5/8 at 200K, might hit 6-7 at 400K
    "dc22": 300000,   # 3/6 at 200K with UCB1
    "lf52": 300000,   # 2/10 at 200K, 10 levels available
    "vc33": 300000,   # 3/7 at 200K
    "ft09": 300000,   # 2/6 at 200K, explores 67K+ states productively
    # Default: 200K
}
